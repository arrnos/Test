...
from config.deep_feature_config import FEATURE_DTYPES, FEATURE_NAMES
...
category_features = [feature_name for feature_name, dtype in zip(FEATURE_NAMES, FEATURE_DTYPES) if dtype == str]
...
def read_feature_values_from_file(feature_name, feature_values_save_path=category_value_path):
    ...
    return list(value_count_dict.values())
...
if __name__ == '__main__':
    ...
    marriageState_value_set = read_feature_values_from_file("marriageState",category_value_path)
    print(marriageState_value_set)
